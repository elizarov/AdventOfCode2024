...
fun main() {
    ...
    fun find(code: String, p: Int): Long {
        ...
        for (tc in code) {
            ...
            val c2 = P2(i0, j1)
            val best = minOf(
                if (c1 != pad.g) find(s1, p + 1) else Long.MAX_VALUE,
                if (c2 != pad.g) find(s2, p + 1) else Long.MAX_VALUE
            )
            i0 = i1
            j0 = j1
            len += best
